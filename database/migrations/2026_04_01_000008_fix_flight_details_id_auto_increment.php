...
{
    /**
     * Shared DB `flight_details` may lack AUTO_INCREMENT on `id`.
     * Inserts then fail with: SQLSTATE[HY000]: 1364 Field 'id' doesn't have a default value.
     */
    public function up(): void
    {
        if (! Schema::hasTable('flight_details')) {
            return;
        }
        ...
        foreach (['created_at', 'updated_at'] as $col) {
            try {
                DB::statement("ALTER TABLE `flight_details` MODIFY `{$col}` TIMESTAMP NULL DEFAULT NULL");
            } catch (\Throwable $e) {
                // ignore if column missing or already compatible
            }
        }

        $pk = DB::select('SHOW KEYS FROM `flight_details` WHERE Key_name = ?', ['PRIMARY']);
        if (empty($pk)) {
            DB::statement('ALTER TABLE `flight_details` ADD PRIMARY KEY (`id`)');
        }
        ...
        DB::statement("ALTER TABLE `flight_details` MODIFY COLUMN `id` {$colType} NOT NULL AUTO_INCREMENT");
    }
